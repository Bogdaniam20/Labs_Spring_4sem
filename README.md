# Лабораторная работа № 7

---

**Курс:** ПнаЯВУ  
**Группа:** 334702  
**Вариант:** № 1  
**ФИО:** Григоренко Б.С.  
---
![](https://i.gifer.com/origin/b6/b63cd17bb3aedad7a15e39ec9c0dcce9_w200.webp)![](https://i.gifer.com/origin/77/77c9d6001ac7ee0ba6e4e313b41aa6dd_w200.webp)![](https://i.gifer.com/origin/2d/2d4adaf81d4a24d3da1a5c93834e56b9_w200.webp)![](https://i.gifer.com/origin/fd/fdf70f5f4989f9c08f033da50c38170e_w200.webp)<br>
---

## Описание

Сделана клиентская часть (UI).
Реализовать UI для добавления, удаления и обновления.

## Изменения и улучшения

Добавлена функциональная клиентская часть

---

## Задание

В рамках лабораторной работы были выполнены следующие задачи:

1. Сделать клиентскую часть (UI) с использованием любых технологий и/или библиотек для GET запроса.
2. Реализовать UI для добавления, удаления и обновления.

---

## Заключение

В рамках данной лабораторной работы была разработана клиентская часть веб-приложения. Основной функционал приложения включает в себя выполнение GET-запроса для получения данных, а также возможность добавления, удаления и обновления записей.

---

## Используемые ресурсы

- [Google Java Style Guide](https://google.github.io/styleguide/javaguide.html)
- [Checkstyle Documentation](https://checkstyle.sourceforge.io/)
- [JMeter Documentation](https://jmeter.apache.org/)
- [Postman Documentation](https://learning.postman.com/)
- [Stack Overflow](https://stackoverflow.com/)

---
